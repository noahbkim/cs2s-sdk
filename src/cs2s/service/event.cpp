...
bool PluginEventService::Load(PluginId id, ISmmAPI* ismm, bool late)
{
    ...
    auto source_2_server = reinterpret_cast<ISource2Server*>(ismm->VInterfaceMatch(ismm->GetServerFactory(), SOURCE2SERVER_INTERFACE_VERSION));
    if (!source_2_server)
    {
    ...
    IToolGameEventAPI* tool_game_event_api = cs2s::vcall<91, IToolGameEventAPI*>(source_2_server);
